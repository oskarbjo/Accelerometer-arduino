...
LSM6DS3Class::LSM6DS3Class(SPIClass& spi, int csPin, int irqPin) :
  ...
  _csPin(csPin),
  _irqPin(irqPin),
  _spiSettings(20E6, MSBFIRST, SPI_MODE0)
{
}
...
int LSM6DS3Class::begin()
{
  ...
  //set the gyroscope control register to work at 416 Hz, 125 dps
  writeRegister(LSM6DS3_CTRL2_G, 0x42);

  // Set the Accelerometer control register to work at 416 Hz, 2G
...
int LSM6DS3Class::readAcceleration(uint16_t& x, uint16_t& y, uint16_t& z)
{
  int16_t data[3];
...
int LSM6DS3Class::readGyroscope(uint16_t& x, uint16_t& y, uint16_t& z)
{
  int16_t data[3];
