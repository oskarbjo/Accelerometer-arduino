...
class LSM6DS3Class {
  public:
    ...
    // Accelerometer
    virtual int readAcceleration(uint16_t& x, uint16_t& y, uint16_t& z); // Results are in G (earth gravity).
    virtual float accelerationSampleRate(); // Sampling rate of the sensor.
    virtual int accelerationAvailable(); // Check for available data from accerometer

    // Gyroscope
    virtual int readGyroscope(uint16_t& x, uint16_t& y, uint16_t& z); // Results are in degrees/second.
